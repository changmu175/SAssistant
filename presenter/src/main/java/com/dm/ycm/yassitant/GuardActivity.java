...
public class GuardActivity extends AppCompatActivity implements CompoundButton.OnCheckedChangeListener,
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        timePreferenceHelper.saveIntegerData("time", 5);
        servicePreferenceHelper = new SharePreferenceHelper(this, "service");
        phoneGuardSPHelper = new SharePreferenceHelper(this, "phone_guard");
        int phoneGuard = phoneGuardSPHelper.getIntegerData("guardIsOpen");
        componentName = new ComponentName(this, LockReceiver.class);
        guardIsOpen = phoneGuard == 1;
        if (guardIsOpen) {
            int lock = phoneGuardSPHelper.getIntegerData("lock");
    ...
    protected void onResume() {
        ...
        if (!LSUtil.checkPermission(this, componentName)) {
            phoneGuardSPHelper.saveIntegerData("lock", 0);
        }
        if (guardIsOpen) {
            int lock = phoneGuardSPHelper.getIntegerData("lock");
            int vibrate = phoneGuardSPHelper.getIntegerData("vibrate");
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        if (buttonView == open_guard) {
            if (isChecked) {
                phoneGuardSPHelper.saveIntegerData("guardIsOpen", 1);
                startService();
//                initView();
                lock_ll.setVisibility(View.VISIBLE);
...
//                setVibrateState(false, vibrateIsOpen);
            ...
            if (isChecked) {
                phoneGuardSPHelper.saveIntegerData("lock", 1);
                if (LSUtil.checkPermission(this, componentName )) {
...
//                setVibrateState(true, true);
                phoneGuardSPHelper.saveIntegerData("vibrate", 1);
            } else {
//                setVibrateState(true, false);
                phoneGuardSPHelper.saveIntegerData("vibrate", 0);
            }
        } else if (buttonView == light) {
            if (isChecked) {
                if (checkFlashlight()) {
                    showCamera();
                    phoneGuardSPHelper.saveIntegerData("light", 1);
                }
            } else {
                phoneGuardSPHelper.saveIntegerData("light", 0);
            }
        }
    }
    ...
    private void stopService() {
        servicePreferenceHelper.saveIntegerData("isWorking", 0);
